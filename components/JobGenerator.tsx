...
const JobGenerator: React.FC = () => {
  ...
  const callCloudMistAI = async (jdText: string, scenarioInstruction: string = '默认使用“刷到宝藏岗位”的语气') => {
    ...
    const systemPrompt = `你是「Talentverse猎头团队」官方合作的内容创作者助手，负责为远程岗位撰写小红书风格的岗位推广文案。
...
【开头】：
(根据【当前场景要求】写开头，语气要自然。默认：今天我给大家带来一家宝藏级“小众外企”～[公司名]！)
...
你必须严格按照以下 JSON 格式返回数据（不要包含 markdown 代码块标记，只返回纯 JSON 字符串）：
{
  ...
  "tags": ["#远程工作", "#内推",  "#居家办公", "#数字游民", "#求职招聘", "#神仙工作","#[JD里的核心技能1]", "#[JD里的核心技能2]", "#[行业关键词]"]
}`;
